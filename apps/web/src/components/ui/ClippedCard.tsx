...
export const ClippedCard = ({
  ...
  return (
    ...
    >
      {width > 0 && height > 0 && (
        <>
          <svg className="pointer-events-none absolute inset-0" width={width} height={height}>
            <defs>
              <clipPath id={clipId}>
                <polygon points={generateBevelPoints(width, height, bevelBase, 0)} />
              </clipPath>
            </defs>
          </svg>
      ...
      <div
        className={cn('z-10 relative h-full w-full p-2 md:p-4', scroll && 'overflow-y-auto')}
        style={{ clipPath: width > 0 && height > 0 ? `url(#${clipId})` : undefined }}
      >
        {children}
      </div>
    </div>
...
ClippedCard.displayName = 'ClippedCard';
